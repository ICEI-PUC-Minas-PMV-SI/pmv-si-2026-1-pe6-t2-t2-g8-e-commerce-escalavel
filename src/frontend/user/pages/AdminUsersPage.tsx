...
const IcoShield = (props: SVGProps<SVGSVGElement>) => (
...
export default function AdminUsersPage() {
  ...
  return (
    <div className="au-root">
          ...
          {loading ? (
            ...
            <table className="au-table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="au-empty">Nenhum usuário encontrado.</td>
                  </tr>
                ) : filtered.map(u => (
                  <tr key={u.id} className={`au-row${!u.active ? ' au-row-inactive' : ''}`}>
                    <td>
                    ...
                    <td>
                      <span className={`au-status au-status-${u.active ? 'on' : 'off'}`}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <div className="au-actions">
                        {u.active ? (
                          <button className="au-act au-act-warn" onClick={() => openModal(u, 'deactivate')} title="Desativar">
                            <IcoPause />
                          </button>
                        ) : (
                          <button className="au-act au-act-ok" onClick={() => openModal(u, 'reactivate')} title="Reativar">
                            <IcoPlay />
                          </button>
                        )}
                        <button className="au-act au-act-danger" onClick={() => openModal(u, 'hard-delete')} title="Excluir">
                          <IcoTrash />
                        </button>
                      </div>
                    </td>
      ...
      {selectedUser && (
        <div className="au-modal-backdrop" onClick={() => !processing && setSelectedUser(null)}>
          <div className="au-modal" onClick={e => e.stopPropagation()}>
            <div className="au-modal-icon"><IcoWarn /></div>
            ...
            <div className="au-modal-actions">
              ...
              <button className={modalMeta[modalAction].confirmClass} onClick={handleConfirm} disabled={processing}>
                {processing ? 'Processando…' : modalMeta[modalAction].confirmLabel}
              </button>
            </div>
          </div>
        </div>
